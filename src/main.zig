        ...
        for (0..10) |_| {
            ...
            switch (first_nibble) {
                ...
                0xF => {
                    ...
                    switch (second_half) {
                        0x0A => {
                            try stdout.print("Fx0A: {x}, key_hold: {any}\n", .{ opcode_value, key_hold });
                            if (key_hold) |key| {
                                try stdout.print("Fx0A key hold: {x}\n", .{key});
                                if (!key_pad[key]) {
                                    try stdout.print("Fx0A key released: {x}\n", .{key});
                                    key_hold = null;
                                    chip8.set_vx(second_nibble, @intCast(key));
                                    chip8.increment_pc();
                                }
                            } else {
                                for (key_pad, 0..) |key_pressed, index| {
                                    if (key_pressed) {
                                        key_hold = index;
                                        try stdout.print("Fx0A key pressed: {x}\n", .{index});
                                        break;
                                    }
                                }
                            }
